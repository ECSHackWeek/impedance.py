...
from .elements import (
    ...
    get_element_parameter_names_and_units_from_name,
)
...
class BaseCircuit:
    ...
    def __init__(self, circuit="", initial_guess=None, constants=None, name=None):
        ...
        circuit: str, optional
            String describing this circuit
        ...
        self.circuit = CircuitGraph._whitespce.sub("", circuit)
    ...
    def fit(
        ...
        if self.initial_guess != []:
            parameters, conf, cov = circuit_fit(
                frequencies,
                impedance,
                self.circuit,
                self.initial_guess,
    ...
    def predict(self, frequencies, use_initial=False):
        ...
        cg = CircuitGraph(self.circuit, self.constants)
        if self._is_fit() and not use_initial:
            return cg.compute(frequencies, *self.parameters_)
        else:
            warnings.warn("Simulating circuit based on initial parameters")
            return cg.compute(frequencies, *self.initial_guess)

    def get_param_names(self):
        """Converts circuit string to names and units"""

        # parse the element names from the circuit string
        names = extract_circuit_elements(self.circuit)
    ...
    def __str__(self):
        ...
        if len(self.constants) > 0:
            ...
            for name, value in self.constants.items():
                ...
                units = circuit_elements[elem].units
                name_parts = name.split("_")
                if len(name_parts) > 1:
                    unit = units[int(name_parts[-1])]
                else:
                    unit = units[0]
    ...
    def load(self, filepath, fitted_as_initial=False):
        ...
        if json_data["Fit"]:
            if fitted_as_initial:
                ...
                if json_data.get("Covariance", None) is not None:
